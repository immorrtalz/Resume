...
document.addEventListener('DOMContentLoaded', () =>
{
	...
	const gradientBlurTopElement = document.querySelector('.gradient-blur-top');
	const whyItemStackInnerContainers = document.querySelectorAll('.why-item-stack-inner-container');

	var scrollProgressInPixels;

	scroll.on('scroll', (args) =>
	{
		const scrollProgress = Math.floor(args.scroll.y / args.limit.y * 100);
		scrollProgressInPixels = Math.floor(args.scroll.y / args.limit.y * document.body.offsetHeight);

		if (typeof args.currentElements['why'] === 'object')
		{
			for (var i = 0; i < whyItemStackInnerContainers.length; i++)
				whyItemStackInnerContainers[i].style.transform = `rotateZ(10deg) translateY(${args.currentElements['why'].progress * (i % 2 == 0 ? 25 : -25)}%)`;
		}
	});
	...
	var navbarLinkCurrentIndex = Array.from(navbarLinks).indexOf(document.querySelector('.navbar-link-current'));
	var prev_navbarLinkCurrentIndex = navbarLinkCurrentIndex;

	setInterval(() =>
	{
		const heroInView = isElementInViewport(heroSectionElement);
		const projectsInView = isElementInViewport(projectsSectionElement);
		const experienceInView = isElementInViewport(experienceSectionElement);
		...
		if (whyInView) navbarLinkCurrentIndex = 3;
		else if (experienceInView) navbarLinkCurrentIndex = 2;
		else if (projectsInView) navbarLinkCurrentIndex = 1;
		else if (heroInView) navbarLinkCurrentIndex = 0;
		else navbarLinkCurrentIndex = -1;

		if (navbarLinkCurrentIndex != prev_navbarLinkCurrentIndex)
		{
			prev_navbarLinkCurrentIndex = navbarLinkCurrentIndex;

			for (var i = 0; i < navbarLinks.length; i++)
			{
				if (i == navbarLinkCurrentIndex) navbarLinks[i].classList.add('navbar-link-current');
				else navbarLinks[i].classList.remove('navbar-link-current');
			}
		}
	}, 200);
	...
	navbarLinks.forEach(navbarLink =>
	{
		navbarLink.addEventListener('click', (e) =>
		{
			...
			scroll.scrollTo(scrollToSections[scrollToSectionIndex], { offset: -150 });
		});
	});
});
...
function isElementInViewport(el)
{
	...
	return ((rect.top >= 0 && rect.top <= window.innerHeight) || (rect.bottom >= 0 && rect.bottom <= window.innerHeight) || (rect.top < 0 && rect.bottom > window.innerHeight));
}